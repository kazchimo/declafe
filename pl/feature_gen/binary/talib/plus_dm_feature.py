...
class PLUS_DMFeature(BinaryFeature):
  ...
  def __init__(self, left: ColLike, right: ColLike, timeperiod: int = 14):
  ...
  def _binary_expr(self, left: pl.Expr, right: pl.Expr) -> pl.Expr:
    return pl.struct(
        [left,
         right]).map(lambda s: talib.PLUS_DM(s[f'{self.left.feature_name}'],
                                             s[f'{self.right.feature_name}'],
                                             timeperiod=self.timeperiod))

  def _feature_names(self) -> list[str]:
    return [f'PLUS_DM({self.timeperiod})({self.left}, {self.right})']
