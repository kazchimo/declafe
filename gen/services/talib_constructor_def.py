...
class Arg:
  ...
  def as_pass_component(self, value: str) -> str:
    if self.type == "ColLike":
      return f"{value}"
    else:
      return f"{self.name}={value}"

  def test_value_name(self) -> str:
    if self.type == "ColLike":
      return f'{self.name}'
    else:
...
class TalibConstructorDef:
  ...
  @property
  def additionl_args(self) -> list[Arg]:
    return [a for a in self.args if a.type != "ColLike"]
